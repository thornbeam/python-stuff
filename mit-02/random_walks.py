...
# plotLocs((UsualDrunk, MasochistDrunk), 1000, 1000)
...
def traceWalk_trialTest(fieldKinds, numSteps, numTrials):
    styleChoice = styleIterator(('b+', 'r^', 'ko'))
    pylab.figure("traceWalk_trialTest")
    pylab.clf()
    for fClass in fieldKinds:
        d = UsualDrunk()
        locs = []
        ...
        pylab.plot(xVals, yVals, curStyle,
                   label = fClass.__name__)
    pylab.title('Final Loc on Walk ('
                + str(numSteps) + ' steps)')
    pylab.xlabel('Steps East/West of Origin')
    pylab.ylabel('Steps North/South of Origin')
    pylab.legend(loc = 'best')

traceWalk_trialTest((Field, OddField), 500, 100)
